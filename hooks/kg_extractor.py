...
KG_FOLDER      = os.getenv("KG_FOLDER", "ClaudeCode")   # subfolder inside vault
...
def write_session_note(kg: dict, vault: Path, folder: Path) -> Path:
    ...
    if rels:
        ...
        for r in rels:
            ...
            lines.append(f"- {from_link} **{r['type']}** {to_link}" + (f" — {r['description']}" if r.get("description") else ""))
        lines.append("")
...
def write_entity_notes(kg: dict, vault: Path, folder: Path):
    ...
    for entity in entities:
        ...
        if outgoing:
            ...
            for r in outgoing:
                to_name = entity_map.get(r["to"], r["to"])
                lines.append(f"- **{r['type']}** [[{slugify(to_name)}|{to_name}]]" + (f" — {r['description']}" if r.get("description") else ""))
            lines.append("")
        ...
        if incoming:
            ...
            for r in incoming:
                from_name = entity_map.get(r["from"], r["from"])
                lines.append(f"- [[{slugify(from_name)}|{from_name}]] **{r['type']}** this" + (f" — {r['description']}" if r.get("description") else ""))
            lines.append("")
...
def main():
    ...
    write_entity_notes(kg, vault, folder)
